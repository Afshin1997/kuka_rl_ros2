...
class JointStateNode:
    def __init__(self):
        ...
        ##########
        self.ee_pose = np.array([0.33434677, -0.33252597, 0.9089085], dtype=np.float32)  # Default home position
        self.ee_orientation = np.array([0.77467585, -0.054232832, 0.61539257, -0.13501088], dtype=np.float32)
    ...
    def initialize_robot_state(self):
        ...
        self.initial_joint_pos = np.array([
            0.7853982,   # iiwa_joint_1
            0.0,      # iiwa_joint_2
            1.3264502,   # iiwa_joint_3
            1.2217305,   # iiwa_joint_4
            1.6057029,   # iiwa_joint_5
            -0.9075712,  # iiwa_joint_6
            1.5707964    # iiwa_joint_7
        ], dtype=np.float32)
    ...
    def load_model_and_normalization(self):
        ...
        # Load model state
        checkpoint = torch.load('/home/user/ros_ws/src/catch_and_throw/input_files/idealpd/model_idealpd.pt', map_location=torch.device('cpu'))
    ...
    def joint_states_callback(self, data):
        with self.lock:
            ...
            observations = np.concatenate((
                pure_action, #7
                self.action_history.flatten(), #14
                dof_pos_scaled_obs, #7
                dof_vel_scaled_obs, #7
                tennisball_pos_obs, #3
                new_point_pos,  # Replaced with offset point #3
