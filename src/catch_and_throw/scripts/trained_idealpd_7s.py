...
class EmpiricalNormalization(nn.Module):
    ...
    def load_state_dict(self, state_dict, strict=True):
        # Check if any normalization keys exist in the checkpoint
        normalization_keys = ['_mean', '_var', '_std', '_mean_cont', '_var_cont', '_std_cont']
        found_keys = [k for k in normalization_keys if k in state_dict]
        
        if not found_keys:
            rospy.logwarn("No normalization keys found in checkpoint! "
                        "Normalization will use default initialization.")
        ...
        # Call parent class to load the state dict
...
class ActorNetwork(nn.Module):
    ...
    def __init__(
        ...
        embedding_dim = 5         # Dimension of the embedding for obs_task
        ...
        layers.append(nn.LayerNorm(actor_hidden_dims[0]))
...
class JointStateNode:
    def __init__(self):
        ...
        self.action_scale = 0.1  # Define scaling factor for actions
        ...
        self.throwing_vel = np.array([1.0, 0.256, 0.5], dtype=np.float32)
        ...
        self.action_history_actor = np.zeros((2, 7), dtype=np.float32)


        ##########
        self.ee_pose = np.array([0.39475822, -0.28232217, 0.85796773], dtype=np.float32)  # Default home position
        self.ee_orientation = np.array([0.7269387, -0.094775364, 0.6735191, -0.09460279], dtype=np.float32)
    ...
    def initialize_robot_state(self):
        ...
        self.initial_joint_pos = np.array([
            45 * np.pi/180,   # iiwa_joint_1
            0 * np.pi/180,    # iiwa_joint_2
            76 * np.pi/180,   # iiwa_joint_3
            70 * np.pi/180,   # iiwa_joint_4
            92 * np.pi/180,   # iiwa_joint_5
            -52 * np.pi/180,  # iiwa_joint_6
            90 * np.pi/180    # iiwa_joint_7
        ], dtype=np.float32)
    ...
    def load_observation_data(self):
        ...
        input_file_path = os.path.join(script_dir, '../../input_files/idealpd/ft_idealpd_7.csv')
    ...
    def load_model_and_normalization(self):
        ...
        # Load model state
        checkpoint = torch.load('/home/user/ros_ws/src/catch_and_throw/input_files/idealpd/model_idealpd_7.pt', map_location=torch.device('cpu'))
    ...
    def joint_states_callback(self, data):
        with self.lock:
            ...
            to_final_target = to_final_target if obs_task_obs[-3] == 1.0 else (np.zeros(3, dtype=np.float32)).flatten()
            to_throwing_pos_scaled = to_throwing_pos_scaled if obs_task_obs[-2] == 1.0 else (np.zeros(3, dtype=np.float32)).flatten()
            to_throwing_vel_scaled = to_throwing_vel_scaled if obs_task_obs[-2] == 1.0 else (np.zeros(3, dtype=np.float32)).flatten()


            self.action_history_actor = np.roll(self.action_history_actor, shift=-1, axis=0)
            self.action_history_actor[-1, :] = self.pure_action
            ...
            observations = np.concatenate((
                pure_action, #7
                self.action_history_actor.flatten(), # 14
                dof_pos_scaled_obs, #7
                dof_vel_scaled_obs, #7
                tennisball_pos_obs, #3
                ee_pos_for_obs,  #3
                tennisball_lin_vel_obs, #3
                ee_lin_vel_scaled, #3
                ee_orientation_obs, #4
                to_final_target_obs, #3
                to_throwing_pos_scaled, #3
                to_throwing_vel_scaled, #3
                obs_task_obs #7
            ))
    ...
    def run(self):
        ...
        script_dir = os.path.dirname(os.path.abspath(__file__))
        output_dir = os.path.join(script_dir, '../../output_files/tm/idealpd_7')
