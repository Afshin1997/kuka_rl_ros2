...
public:
    ...
    void compute_action_and_publish() {
        ...
        action_history_[-1] = pure_action_;
        ...
        // Prepare observation tensor (67 elements total)
        ...
        // Log task state changes periodically
        if (t_ % 50 == 0) {
            int current_task_index = -1;
            for (int i = 0; i < 7; i++) {
                if (current_task_[i].item<float>() == 1.0) {
            ...
            RCLCPP_INFO(this->get_logger(), "t=%d, Task: %s, Ball: [%.3f, %.3f, %.3f], Dist: %.3f", 
                       t_, task_names[current_task_index].c_str(),
                       tennisball_pos_[0].item<float>(), 
                       tennisball_pos_[1].item<float>(), 
                       tennisball_pos_[2].item<float>(),
                       torch::norm(tennisball_pos_ - ee_pose_, 2).item<float>());
        }
    ...
    void destroy_node() {
        ...
        std::string output_dir = "/home/user/kuka_rl_ros2/src/catch_and_throw/output_files/tm/optitrack_reduced";
