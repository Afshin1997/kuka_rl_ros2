...
from .actuators import ImplicitActuatorCfg, IdealPDActuatorCfg
...
from .actuators.actuator_impedance import JointImpedanceActuatorCfg
...
class EventCfg:
    ...
    robot_physics_material = EventTerm(
        ...
        params={
            ...
            "static_friction_range": (0.5, 0.7),  # High for good contact
            "dynamic_friction_range": (0.5, 0.7),  # High for good contact
            "restitution_range": (0.05, 0.15),  # Very low to minimize bounce
            "num_buckets": 250,
        },
    )
    ...
    object_physics_material = EventTerm(
        ...
        params={
            ...
            "static_friction_range": (0.4, 0.6),  # Moderately high for good grip
            "dynamic_friction_range": (0.3, 0.5),  # Moderately high for good grip
            "restitution_range": (0.2, 0.4),  # Low to minimize bounce
            "num_buckets": 250,
        },
    )
...
@configclass
class Lbr_iiwaFourStatesEnvCfg(DirectRLEnvCfg):

    # env
    episode_length_s = 20  # 500 timesteps
    decimation = 2
    action_space = 7
    ...
    robot = ArticulationCfg(
        ...
        # actuators={
        #     "ImpedanceActuatorGroup": JointImpedanceActuatorCfg(
        #         joint_names_expr=[".*"],  # or a subset of joints
        #         stiffness=300.0,
        #         damping_ratio=1.0,
        #         command_type="p_abs",
        #         effort_limit=150.0,
        #         inertial_compensation=True,   # Enable inertia compensation
    ...
    tasks = ['Catching_Ball', 'Stabilizing_Ball', 'Stabilized_Ball', 'Moving_to_target_pos', 'Stabilized_at_Target']

    # reward scales
    dist_reward_scale = 2.0
    orient_reward_scale = 0.55
    relative_velocity_reward_scale = 1.1
    stability_reward_scale = 1.0
    gravity_compensation_reward_scale = 0.7
    dist_to_target_pos_reward_scale = 2.0 # change to 40
    task_reward_scale = 1.0
    dof_at_limit_cost_scale = 1.0
    dof_vel_lim_penalty_scale = 1.0
    action_sign_scale = 0.015
    joint_accel_scale = 8.0e-9
    joint_jerk_scale = 4.0e-13

    # at every time-step add gaussian noise + bias. The bias is a gaussian sampled at reset
    # action_noise_model: NoiseModelWithAdditiveBiasCfgAction = NoiseModelWithAdditiveBiasCfgAction(
    #     noise_cfg=GaussianNoiseCfg(mean=0.0, std=0.0002, operation="add"),
    #     bias_noise_cfg=GaussianNoiseCfg(mean=0.0, std=0.0005, operation="abs"),
    # )
    # at every time-step add gaussian noise + bias. The bias is a gaussian sampled at reset
    ...
    action_noise_model: NoiseModelWithAdditiveBiasCfgAction = NoiseModelWithAdditiveBiasCfgAction(
        noise_cfg=GaussianNoiseCfg(
            mean=1.0,        # Center scaling factor around 1.0
            std=0.0002,      # Magnitude of perturbation around 1.0
            operation="scale"
        ),
        bias_noise_cfg=GaussianNoiseCfg(
            mean=1.0,        # Bias scaling factor also around 1.0
            std=0.00005,      # Or any suitable STD for bias scaling
            operation="scale"
        ),
    )
...
class Lbr_iiwaFourStatesEnv(DirectRLEnv):
    ...
    cfg: Lbr_iiwaFourStatesEnvCfg

    def __init__(self, cfg: Lbr_iiwaFourStatesEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # print("self.robot_dof_lower_limits", self.robot_dof_lower_limits)
        # print("self.robot_dof_upper_limits", self.robot_dof_upper_limits)
        ...
        # self.robot_dof_targets = torch.tensor([15 * math.pi / 180, -90 * math.pi / 180, 90 * math.pi / 180,
        #                                        90 * math.pi / 180, 90 * math.pi / 180, -90 * math.pi / 180,
        #                                        75 * math.pi / 180], dtype=torch.float, device=self.device).repeat((self.num_envs, 1))
        ...
        self.tennisball_rot = torch.zeros((self.num_envs,4), device=self.device)
        ...
        self.final_target_pos = torch.tensor([0.4, -0.4, 0.46], dtype=torch.float, device=self.device).repeat((self.num_envs, 1))
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        # self.actions = F.softshrink(actions.clone(), lambd=0.05)
        self.actions = actions.clone()
        

        targets = self.robot_dof_pos + self.dt * self.actions * 18
    ...
    def _apply_action(self):
        ...
        # self._robot.set_joint_velocity_target(self.robot_dof_vel_targets)
        self._robot.set_joint_effort_target(self.torque_target)
        
        # print("target_pos", self.robot_dof_targets)
        # print("target_vel", self.robot_dof_vel_targets)
        self._robot.write_data_to_sim()

        # print('£££££££££££££££££££££££££££££££')
        # print("torques", self._robot.data.applied_torque[0])
        # print('self.robot_dof_vel', self._robot.data.joint_vel[0])
        # print('self.dof_pos_scaled', self.dof_pos_scaled.abs()> 0.97)
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        conditions = [
            ...
            # torch.any((self._robot.data.joint_acc.abs() > self.acc_lim), dim=-1),
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        init_ball_state = self._tennisball.data.default_root_state.clone()[env_ids, :3]
        final_ball_state = torch.tensor([0.2, -0.35, 0.9], dtype=torch.float, device=self.device).repeat(len(env_ids), 1)

        random_ball_states = self.random_throw(init_ball_state, final_ball_state)
        # print(random_ball_states)
        ...
        # pos_noise = self.random_ranges(env_ids)
        # self.tennisball_default_state[:, 0:3] = (
        #     self.tennisball_default_state[:, 0:3] + pos_noise + self.scene.env_origins[env_ids]
        # )

        # lin_vel_noise = sample_uniform(-0.1, 0.1, (len(env_ids), 3), device=self.device)
        # lin_vel_noise[:, 1] = 0.0
        # # Add the noise only to x and z components
        # self.tennisball_default_state[:, 7:10] = (
        #     self.tennisball_default_state[:, 7:10] + lin_vel_noise
        # )
        ...
        # print("endeffector",self.end_effector_pos[env_ids])
        # print("ball",self.tennisball_pos[env_ids])


    def _get_observations(self) -> dict:
        
        to_target = self.tennisball_pos - self.end_effector_pos
        to_target_vel = self.tennisball_lin_vel - self.end_effector_lin_vel
        
        self.to_final_target = self.final_target_pos - self.tennisball_pos
        condition = (self.task[:, -1] == 1.0) | (self.task[:, -2] == 1.0)
        self.to_final_target = torch.where(
            condition.unsqueeze(-1),  # Shape: (num_envs, 1)
            (self.final_target_pos - self.tennisball_pos),  # Shape: (num_envs, 1)
            torch.zeros((self.num_envs, 3), device=self.device)  # Shape: (num_envs, 1)
        )
        ...
        # )
        # return {"policy": obs}
        # print(self._robot.data.joint_acc)
        # print("obs_task", obs_task[0])
        obs = torch.cat(
            (   self.actions, #7
                self.dof_pos_scaled, # 7
                self._robot.data.joint_vel * self.cfg.dof_velocity_scale, # 7
                self._robot.data.joint_acc * self.cfg.dof_acc_scale,
                # self._robot.data.joint_acc * self.cfg.dof_acc_scale, # 7
    ...
    def _compute_intermediate_values(self, env_ids: torch.Tensor | None = None):
        ...
        self.end_effector_pos = self._robot.data.body_pos_w[:, self.end_effector_idx] - self.scene.env_origins
        self.end_effector_rot = self._robot.data.body_quat_w[:, self.end_effector_idx]
        self.end_effector_rot = quat_unique(self.end_effector_rot)
        self.end_effector_lin_vel = self._robot.data.body_lin_vel_w[:, self.end_effector_idx]
        ...
        self.tennisball_rot = self._tennisball.data.root_quat_w
        ...
        # Calculate jerk
        self.joint_jerk = torch.sum((self._robot.data.joint_acc - self.prev_joint_acc) / self.dt) ** 2
        ...
        if can_revert_to_catching_ball.any():
            ...
            self.revert_penalty[can_revert_to_catching_ball] = -0.4

        # Conditions for 'Stabilized_Ball'
        distance_condition = (distance < 0.12) & (self.rel_position_termination < self.cfg.tennisball_radius * 1.5)
        velocity_condition = torch.norm(self.tennisball_lin_vel, p=2, dim=-1) < 0.2
        ...
        # print("velocity_condition",torch.norm(self.tennisball_lin_vel[0], p=2, dim=-1))
        # print("distance_condition", distance_condition[0])
        # print("velocity_condition", velocity_condition[0])
        # print("rel_velocity_condition", rel_velocity_condition[0])
        # print('self.stabilized_ball_counter', self.stabilized_ball_counter[0])
        
        self.moving_to_target_pos = (self.stabilized_ball_counter > 250) & distance_condition & rel_velocity_condition
        ...
        final_rel_dis_cond = (distance < 0.12) & (self.rel_position_termination < self.cfg.tennisball_radius * 1.5)
        final_vel_cond = torch.norm(self.tennisball_lin_vel, p=2, dim=-1) < 0.10
        final_rel_velocity_condition = torch.norm(self.tennisball_lin_vel - self.end_effector_lin_vel, p=2, dim=-1) < 0.2
        final_target_dist_reward = dist_to_target_pos < 0.05
        ...
        self.finished_episode = self.stabilized_ball_counter_at_target > 250
    ...
    def _compute_rewards(
        ...
        end_effector_vel_normalized = F.normalize(end_effector_lin_vel, p=2, dim=-1)
        ...
        normalized_rel_pos = F.normalize(self.end_effector_pos - self.tennisball_pos, p=2, dim=-1)
        ...
        finished_reward[self.finished_episode] = 30

        # Initialize a mask for each condition
        final_target_reached_mask = self.stabilized_at_target
        # If not final target reached, check moving to target
        moving_to_target_mask = self.moving_to_target_pos & ~final_target_reached_mask
        # If not final target reached or moving to target, check stabilized ball
        stabilized_ball_mask = self.stabilized_ball & ~final_target_reached_mask & ~moving_to_target_mask
        # If not in any above states, check catched ball
        catched_ball_mask = self.catched_ball & ~final_target_reached_mask & ~moving_to_target_mask & ~stabilized_ball_mask
        ...
        catched_ball_reward_mask = ~self.given_task_reward['catched_ball']
        alive_reward = torch.ones((self.num_envs), device=self.device) * 0.2
        action_penalty = -torch.sum(actions**2, dim=-1)

        if final_target_reached_mask.any():
            task_reward[final_target_reached_reward_mask] += 25.0
            self.given_task_reward['reached_final_target'][final_target_reached_reward_mask] = True
            orient_reward[final_target_reached_mask] = 1.0 # 1.0 some additional reward
            dist_reward[final_target_reached_mask] = 1.6 * dist_reward[final_target_reached_mask]
            r_v_reward[final_target_reached_mask] = 1.6 * r_v_reward[final_target_reached_mask]
            stability_reward[final_target_reached_mask] = 1.6 / (1.0 + tennisball_linear_vel_mag[final_target_reached_mask])
            gravity_compensation_reward[final_target_reached_mask] = 1.6 * gravity_compensation[final_target_reached_mask]
            dist_to_target_pos_reward[final_target_reached_mask] = (1.2 / (1.0 + 5.0 * (dist_to_target_pos[final_target_reached_mask])**2))
            termination_penalty[final_target_reached_mask] *= 1.6
            alive_reward[final_target_reached_mask] += 1.4
            # action_penalty[final_target_reached_mask] *= 2.0
        ...
        if moving_to_target_mask.any():
            ...
            gravity_compensation_reward[moving_to_target_mask] = 1.3 * gravity_compensation[moving_to_target_mask]
            dist_to_target_pos_reward[moving_to_target_mask] = (1.0 / (1.0 + 5.0 * (dist_to_target_pos[moving_to_target_mask])**2))
            termination_penalty[moving_to_target_mask] *= 1.5
        ...
        if stabilized_ball_mask.any():
            ...
            # action_penalty[stabilized_ball_mask] *= 2.0
        ...
        if catched_ball_mask.any():
            ...
            stability_reward[catched_ball_mask] = 1.1 / (1.0 + tennisball_linear_vel_mag[catched_ball_mask]**2)
        ...
        joint_accel = torch.sum(self._robot.data.joint_acc ** 2)
        ...
        rewards = (
            ...
            - action_sign_change_penalty * action_sign_scale
            - joint_accel * joint_accel_scale
            - self.joint_jerk * joint_jerk_scale
            + alive_reward
            + self.revert_penalty
            # + action_penalty * 0.01
            + finished_reward
        ...
        self.extras["log"] = {
            ...
            "action_sign_change_penalty": (- action_sign_change_penalty * action_sign_scale).mean(),
            "joint_accel": (- joint_accel * joint_accel_scale).mean(),
            "joint_jerk": (- self.joint_jerk * joint_jerk_scale).mean(),
