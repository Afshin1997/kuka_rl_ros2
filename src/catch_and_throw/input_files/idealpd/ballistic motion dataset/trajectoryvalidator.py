...
class TrajectoryValidator:
    ...
    def load_trajectories(self, file_paths):
        """Load trajectory CSV files up to appropriate stopping condition"""
        trajectories = {}
        ...
        for file_path in file_paths:
            ...
            try:
                ...
                required_cols = ['ball_ema_vx', 'ball_pos_z', 'ball_pos_y']
                missing_cols = [col for col in required_cols if col not in df.columns]
                if missing_cols:
                    print(f"Warning: Missing columns {missing_cols} in {name}. Available columns: {df.columns.tolist()}")
                    trajectories[name] = df
                    continue
                
                # Find stopping conditions
                # 1. ball_ema_vx is zero (or very close to zero)
                zero_vx_indices = df[abs(df['ball_ema_vx']) < 1e-6].index.tolist()
                ...
                print(f"File {name}: Found {len(zero_vx_indices)} zero vx rows, " +
                    f"{len(zero_pz_indices)} low z rows, {len(bad_py_indices)} bad y range rows")
                ...
                if cutoff_candidates:
                    ...
                    print(f"File {name}: No stopping conditions met, keeping all {len(df)} rows")
                
                # Additional check: if we still have data in the bad y range, warn about it
                remaining_bad_y = df[(df['ball_pos_y'] >= -0.3)]
                if len(remaining_bad_y) > 0:
                    print(f"Warning: File {name} still contains {len(remaining_bad_y)} rows with y in [-0.3, -0.2] range")
                    print(f"Y range in remaining data: [{df['ball_pos_y'].min():.3f}, {df['ball_pos_y'].max():.3f}]")
    ...
    def plot_validation_summary(self, trajectories, results):
        ...
        # Plot 2: Component-wise position analysis
        ...
        # Plot 3: Component-wise velocity analysis with expected ranges (first row only)
...
if __name__ == "__main__":
    ...
    # Method 1: Load all CSV files in current directory
    file_paths = glob.glob("*.csv")
    # file_paths = ['26.csv', '27.csv', '28.csv', '29.csv', '30.csv']
    
    # Method 2: Load all CSV files in a specific directory
    # file_paths = glob.glob("/path/to/your/directory/*.csv")
    
    # Method 3: Load CSV files matching a pattern (e.g., numbered files)
    # file_paths = glob.glob("[10-20].csv")  # Files starting with numbers
    
    # Method 4: Load CSV files recursively from subdirectories
    # file_paths = glob.glob("**/*.csv", recursive=True)
    
    # Method 5: Using os.listdir() for more control
    # file_paths = [f for f in os.listdir('.') if f.endswith('.csv')]
    
    # Method 6: Filter by file size or other criteria
    # file_paths = [f for f in glob.glob("*.csv") if os.path.getsize(f) > 1000]  # Files > 1KB
    
    # Method 7: Exclude specific files
    # exclude_files = ['backup.csv', 'temp.csv']
    # file_paths = [f for f in glob.glob("*.csv") if f not in exclude_files]
    
    # Sort the files for consistent ordering (optional)
    file_paths.sort()
    
    # Alternative sorting methods:
    # file_paths.sort(key=lambda x: int(x.split('.')[0]) if x.split('.')[0].isdigit() else float('inf'))  # Sort numerically
