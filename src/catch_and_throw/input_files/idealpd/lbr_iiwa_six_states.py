...
from .actuators import IdealPDActuatorCfg
...
headers += [f'obs_task_{i}' for i in range(5)]
...
class EventCfg:
    ...
    robot_physics_material = EventTerm(
        ...
        params={
            ...
            # "asset_cfg": SceneEntityCfg("robot", body_names="lbr_iiwa_link_7"),
    ...
    reset_gravity = EventTerm(
        ...
        mode="reset",
        params={
            "gravity_distribution_params": ([0.0, 0.0, -0.05], [0.0, 0.0, 0.05]),
            "operation": "add",
...
class Lbr_iiwaSixStatesEnvCfg(DirectRLEnvCfg):
    ...
    lin_vel_scale = 0.15
    to_final_target_scale = 1.0
    to_throwing_pos_scale = 3.0
    to_throwing_vel_scale = 2.0
    ...
    robot = ArticulationCfg(
        ...
        actuators={
            "A1": IdealPDActuatorCfg(
                ...
                velocity_limit=0.99 * 98.0 * math.pi / 180,  # ≈ 1.713 rad/s
                effort_limit=176.0,
                stiffness=400,
                damping=28,
            ),
            ...
            "A3": IdealPDActuatorCfg(
                ...
                velocity_limit=0.99 * 100.0 * math.pi / 180,  # ≈ 1.745 rad/s
                effort_limit=110.0,
                stiffness=400,
                damping=28,
            ),
            "A4": IdealPDActuatorCfg(
                ...
                velocity_limit=0.99 * 130.0 * math.pi / 180,  # ≈ 2.268 rad/s
                effort_limit=110.0,
                stiffness=400,
                damping=28,
            ),
            "A5": IdealPDActuatorCfg(
                ...
                velocity_limit=0.99 * 140.0 * math.pi / 180,  # ≈ 2.443 rad/s
                effort_limit=110.0,
                stiffness=400,
                damping=28,
            ),
            "A6": IdealPDActuatorCfg(
                ...
                velocity_limit=0.99 * 180.0 * math.pi / 180,  # ≈ 3.142 rad/s
                effort_limit=40.0,
                stiffness=100,
                damping=14,
            ),
    ...
    tasks = ['Catching_Ball', 'Stabilizing_Ball', 'Stabilized_Ball', 'Moving_to_target_pos', 'Stabilized_at_Target', 'Throwing_Ball']
    ...
    gravity_compensation_reward_scale = 0.7
    dist_to_target_pos_reward_scale = 1.0 # change to 40
    task_reward_scale = 1.0
    dof_at_limit_cost_scale = 0.2
    dof_vel_lim_penalty_scale = 0.2
    action_rate_scale = 0.1
    joint_accel_scale = 1.0e-4
    joint_jerk_scale = 2.0e-8
    to_throwing_vel_reward_scale = 4.0
...
class Lbr_iiwaSixStatesEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: Lbr_iiwaSixStatesEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.acc_lim = torch.tensor([490, 490, 500, 650, 700, 900, 900], dtype=torch.float, device=self.device).repeat((self.num_envs, 1))
        ...
        # self.prev_target = torch.tensor([45 * math.pi / 180, 0 * math.pi / 180, 76 * math.pi / 180,
        #                                        70 * math.pi / 180, 92 * math.pi / 180, -52 * math.pi / 180,
        #                                        90 * math.pi / 180], dtype=torch.float, device=self.device).repeat((self.num_envs, 1))
        

        self.stiffness = torch.tensor([400, 400, 400, 400, 400, 100, 100], dtype=torch.float, device=self.device).repeat((self.num_envs, 1))
        self.damping = torch.tensor([28, 28, 28, 28, 28, 14, 14], dtype=torch.float, device=self.device).repeat((self.num_envs, 1))
        ...
        self.prev_joint_pos = torch.tensor([45 * math.pi / 180, 0 * math.pi / 180, 76 * math.pi / 180,
                                               70 * math.pi / 180, 92 * math.pi / 180, -52 * math.pi / 180,
                                               90 * math.pi / 180], dtype=torch.float, device=self.device).repeat((self.num_envs, 1))
        self.joint_vel = torch.zeros((self.num_envs, 7), device=self.device)
        # self.prev_target = torch.zeros((self.num_envs, 7), device=self.device)
        ...
        self.end_effector_lin_vel = torch.zeros((self.num_envs, 3), device=self.device)
        self.tennisball_pos = torch.zeros((self.num_envs,3), device=self.device)
        ...
        self.final_target_pos = torch.tensor([-0.7, -0.16, 0.56], dtype=torch.float, device=self.device).repeat((self.num_envs, 1))
        ...
        self.given_task_reward = {
            'stabilized_at_target': torch.zeros(self.num_envs, dtype=torch.bool, device=self.device),
            'reached_final_target': torch.zeros(self.num_envs, dtype=torch.bool, device=self.device),
            'stabilized_ball': torch.zeros(self.num_envs, dtype=torch.bool, device=self.device),
            'catched_ball': torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)
        }
        ...
        self.last_actions_2 = torch.zeros((self.num_envs, 7), dtype=torch.float, device=self.device)
        self.last_actions_3 = torch.zeros((self.num_envs, 7), dtype=torch.float, device=self.device)
        ...
        # self.register_buffer("b", torch.tensor(b, dtype=torch.float32, device=self.device))
        # self.register_buffer("a", torch.tensor(a, dtype=torch.float32, device=self.device))
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        self.actions = (self.cfg.act_moving_average * self.last_actions) + ((1 - self.cfg.act_moving_average) * self.actions)
        
        targets = self.robot_dof_pos + self.actions * 0.08

        # targets =self.actions * self.robot_dof_upper_limits

        # targets = self.lowpass_filter(targets, cutoff=25, fs=250)
        # targets = (self.cfg.act_moving_average * self.robot_dof_pos) + ((1 - self.cfg.act_moving_average) * targets)
        # targets = (targets + self.prev_target) / 2
        ...
        torques = torch.bmm(mass_matrix, des_acc.unsqueeze(-1)).squeeze(-1) + gravity_forces
        self.torque_target = torch.clip(torques, min=-self.effort_limit, max=self.effort_limit)
        # self.prev_target = targets
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        conditions = [
            ...
            # torch.any((self.actions - self.last_actions).abs() > 0.4, dim=-1),
            self.end_effector_pos[:, 2] < 0.30,
            self.rel_position_termination < 0,
            self.tennisball_pos[:, 2] - self.end_effector_pos[:, 2] < -0.1,
        ...
        truncated = (self.episode_length_buf >= self.max_episode_length - 1) | (self.finished_episode)
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        return self._compute_rewards(
            ...
            self.cfg.gravity_compensation_reward_scale,
            ...
            self.cfg.to_throwing_vel_reward_scale,
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        # self.prev_target[env_ids] = joint_pos
        ...
        self.torque_target[env_ids] = joint_effort
        ...
        estimated_catching_pos = torch.tensor([0.2, -0.35, 0.9], dtype=torch.float, device=self.device).repeat(len(env_ids), 1)

        random_ball_states = self.random_throw(self.init_ball_state, estimated_catching_pos)
        ...
        for key in self.given_task_reward:
            self.given_task_reward[key][env_ids] = False

        self.last_actions[env_ids] = torch.zeros((len(env_ids), 7), dtype=torch.float, device=self.device)
        self.last_actions_2[env_ids] = torch.zeros((len(env_ids), 7), dtype=torch.float, device=self.device)
        self.last_actions_3[env_ids] = torch.zeros((len(env_ids), 7), dtype=torch.float, device=self.device)
    ...
    def _get_observations(self) -> dict:
        ...
        # self.joint_jerk = (self.joint_acc - self.prev_joint_acc) / self.dt

        # print("self._robot.data.joint_pos", self.robot_dof_pos[0])
        # print("self.prev_joint_pos", self.prev_joint_pos[0])
        # print("self._robot.data.joint_vel", self._robot.data.joint_vel[0])
        # print("self.joint_vel", self.joint_vel[0])
        # print("self._robot.data.joint_acc", self._robot.data.joint_acc[0])
        # print("self.joint_acc", self.joint_acc[0])
        # print("self.joint_jerk", self.joint_jerk[0])
        

        self.prev_joint_vel = self.joint_vel
        # self.prev_joint_acc = self.joint_acc
        ...
        obs_actor = torch.cat(
            (   self.actions, #7
                ...
                # self.joint_acc * self.cfg.dof_acc_scale,
    ...
    def _compute_intermediate_values(self, env_ids: torch.Tensor | None = None):
        ...
        self.catched_ball = (distance < 0.12) & (self.rel_position_termination < self.cfg.tennisball_radius * 1.5)
        ...
        if can_revert_to_catching_ball.any():
            ...
            self.revert_penalty[can_revert_to_catching_ball] = -1.0

        # Conditions for 'Stabilized_Ball'
        distance_condition = (distance < 0.12) & (self.rel_position_termination < self.cfg.tennisball_radius * 1.2)
        velocity_condition = torch.norm(self.tennisball_lin_vel, p=2, dim=-1) < 0.1
        rel_velocity_condition = torch.norm(self.tennisball_lin_vel - self.end_effector_lin_vel, p=2, dim=-1) < 0.15
        ...
        if can_revert_to_stabilizing_ball.any():
            ...
            self.revert_penalty[can_revert_to_stabilizing_ball] = -1.0
        
        rel_velocity_condition_to_taret = torch.norm(self.tennisball_lin_vel - self.end_effector_lin_vel, p=2, dim=-1) < 0.1

        self.moving_to_target_pos = (self.stabilized_ball_counter > 50) & distance_condition & rel_velocity_condition_to_taret
        ...
        if can_revert_to_stabilized_ball.any():
            ...
            self.revert_penalty[can_revert_to_stabilized_ball] = -1.0
        ...
        final_rel_dis_cond = (distance < 0.12) & (self.rel_position_termination < self.cfg.tennisball_radius * 1.2)
        ...
        # print("final_rel_dis_cond", final_rel_dis_cond[0])
        # print("final_vel_cond", final_vel_cond[0])
        # print("final_rel_velocity_condition", final_rel_velocity_condition[0])
        # print("final_target_dist_reward", final_target_dist_reward[0])
        ...
        if can_revert_to_moving_to_target.any():
            ...
            self.revert_penalty[can_revert_to_moving_to_target] = -1.0

        self.throwing_ball = (self.stabilized_ball_counter_at_target > 50) & final_rel_dis_cond
        ...
        self.finished_episode = self.throwing_ball & (self.throwing_pos[:, 0] - self.tennisball_pos[:, 0] < 0.0) & (torch.norm(self.throwing_pos - self.tennisball_pos, p=2, dim=-1) <= 0.1)
        self.termination_state = (self.throwing_termination & ~self.throwing_ball) | (self.throwing_ball & (self.throwing_pos[:, 0] - self.tennisball_pos[:, 0] < 0.0) & (torch.norm(self.throwing_pos - self.tennisball_pos, p=2, dim=-1) > 0.1))
    ...
    def _compute_rewards(
        ...
        gravity_compensation_reward_scale,
        ...
        to_throwing_vel_normalized_reward = torch.zeros((self.num_envs), device=self.device)

        # Initialize a mask for each condition
        throwing_ball_mask = self.throwing_ball

        final_target_reached_mask = self.stabilized_at_target & ~throwing_ball_mask
        # If not final target reached, check moving to target
        moving_to_target_mask = self.moving_to_target_pos & ~final_target_reached_mask & ~throwing_ball_mask
        # If not final target reached or moving to target, check stabilized ball
        stabilized_ball_mask = self.stabilized_ball & ~final_target_reached_mask & ~moving_to_target_mask & ~throwing_ball_mask
        # If not in any above states, check catched ball
        catched_ball_mask = self.catched_ball & ~final_target_reached_mask & ~moving_to_target_mask & ~stabilized_ball_mask & ~throwing_ball_mask

        stabilied_at_target_reward_mask = throwing_ball_mask & ~self.given_task_reward['stabilized_at_target']
        final_target_reached_reward_mask = final_target_reached_mask & ~self.given_task_reward['reached_final_target']
        stabilized_ball_reward_mask = moving_to_target_mask & ~self.given_task_reward['stabilized_ball']
        catched_ball_reward_mask = stabilized_ball_mask & ~self.given_task_reward['catched_ball']
        alive_reward = torch.ones((self.num_envs), device=self.device) * 0.2
        ...
        action_change_1 = 1.0 * torch.sum((actions - self.last_actions).abs() > 0.45, dim=-1)
        # print(action_change_1.shape)

        torch.norm(self.robot_dof_targets - self.last_actions, p=2, dim=-1)
        action_change_2 = 1.0 * torch.norm(self.robot_dof_targets - 2 * self.last_actions + self.last_actions_2, p=2, dim=-1)
        action_change_3 = 1.0 * torch.norm(self.robot_dof_targets - 3 * self.last_actions + 3 * self.last_actions_2 - self.last_actions_3, p=2, dim=-1)
        action_change_penalty = action_change_1 ** 2
        # print("action_change_penalty", action_change_penalty)
        
        self.last_actions_3 = self.last_actions_2.clone()
        self.last_actions_2 = self.last_actions.clone()
        self.last_actions = actions.clone()
        

        # print("actions:", actions[0]) 
        # print("self.last_actions:", self.last_actions[0])
        # print("self.last_actions_2:", self.last_actions_2[0])
        # joint_accel = torch.sum(self._robot.data.joint_acc ** 2, dim=-1)

        
        if throwing_ball_mask.any():
            task_reward[stabilied_at_target_reward_mask] += 55.0
            self.given_task_reward['stabilized_at_target'][stabilied_at_target_reward_mask] = True
            orient_reward[throwing_ball_mask] = 1.9 # 1.0 some additional reward
            dist_reward[throwing_ball_mask] = 1.9
            r_v_reward[throwing_ball_mask] = 1.9
            stability_reward[throwing_ball_mask] = 1.9
            gravity_compensation_reward[throwing_ball_mask] = 1.9
            dist_to_target_pos_reward[throwing_ball_mask] = 5.5
            # termination_penalty[throwing_ball_mask] *= 1.9
            alive_reward[throwing_ball_mask] += 4.2
            # self.joint_acc_penalty[throwing_ball_mask] *= 0.5
            # self.joint_jerk_penalty[throwing_ball_mask] *= 0.5
            # dof_vel_lim_penalty[throwing_ball_mask] = 0.0
            to_throwing_vel_normalized_reward[throwing_ball_mask] = (self.throwing_vel_norm[throwing_ball_mask] * F.normalize(self.tennisball_lin_vel[throwing_ball_mask], p=2, dim=-1)).sum(dim=-1)
            to_throwing_vel_reward[throwing_ball_mask] = (5.0 / (1.0 + 0.2 * torch.norm(self.throwing_vel[throwing_ball_mask] - self.tennisball_lin_vel[throwing_ball_mask], p=2, dim=-1)**2))
            to_throwing_pos_reward[throwing_ball_mask] = 5.0 / (1.0 + 5.0 * torch.norm(self.throwing_pos[throwing_ball_mask] - self.tennisball_pos[throwing_ball_mask], p=2, dim=-1)**2)
            # to_throwing_pos_reward[throwing_ball_mask] = to_throwing_pos_reward[throwing_ball_mask] * to_throwing_vel_reward[throwing_ball_mask]

        if final_target_reached_mask.any():
            task_reward[final_target_reached_reward_mask] += 45.0
            self.given_task_reward['reached_final_target'][final_target_reached_reward_mask] = True
            orient_reward[final_target_reached_mask] = 1.7 # 1.0 some additional reward
            dist_reward[final_target_reached_mask] = 1.7 * dist_reward[final_target_reached_mask]
            r_v_reward[final_target_reached_mask] = 1.7 * r_v_reward[final_target_reached_mask]
            stability_reward[final_target_reached_mask] = 1.7 / (1.0 + tennisball_linear_vel_mag[final_target_reached_mask])
            gravity_compensation_reward[final_target_reached_mask] = 1.7 * gravity_compensation[final_target_reached_mask]
            dist_to_target_pos_reward[final_target_reached_mask] = (4.5/ (1.0 + 10.0 * (dist_to_target_pos[final_target_reached_mask])**2))
            # termination_penalty[final_target_reached_mask] *= 1.7
            alive_reward[final_target_reached_mask] += 3.2           

        if moving_to_target_mask.any():
            task_reward[stabilized_ball_reward_mask] += 15.0
            self.given_task_reward['stabilized_ball'][stabilized_ball_reward_mask] = True
            orient_reward[moving_to_target_mask] = 1.5
            dist_reward[moving_to_target_mask] = 1.5 * dist_reward[moving_to_target_mask]
            r_v_reward[moving_to_target_mask] = 1.5 * r_v_reward[moving_to_target_mask]
            stability_reward[moving_to_target_mask] = 1.5
            gravity_compensation_reward[moving_to_target_mask] = 1.5 * gravity_compensation[moving_to_target_mask]
            dist_to_target_pos_reward[moving_to_target_mask] = (3.5 / (1.0 + 10.0 * (dist_to_target_pos[moving_to_target_mask])**2))
            # termination_penalty[moving_to_target_mask] *= 1.3
            alive_reward[moving_to_target_mask] += 2.2

        if stabilized_ball_mask.any():
            task_reward[catched_ball_reward_mask] += 5.0
            self.given_task_reward['catched_ball'][catched_ball_reward_mask] = True
            orient_reward[stabilized_ball_mask] = 1.3
            dist_reward[stabilized_ball_mask] = 1.3 * dist_reward[stabilized_ball_mask]
            r_v_reward[stabilized_ball_mask] = 1.3 * r_v_reward[stabilized_ball_mask]
            stability_reward[stabilized_ball_mask] = 1.3 / (1.0 + tennisball_linear_vel_mag[stabilized_ball_mask])
            gravity_compensation_reward[stabilized_ball_mask] = 1.3 * gravity_compensation[stabilized_ball_mask]
            # termination_penalty[stabilized_ball_mask] *= 1.3 
            alive_reward[stabilized_ball_mask] += 1.2

        if catched_ball_mask.any():
            orient_reward[catched_ball_mask] = 1.0
            dist_reward[catched_ball_mask] = 1.1 * dist_reward[catched_ball_mask]
            r_v_reward[catched_ball_mask] = 1.1 * r_v_reward[catched_ball_mask]
            stability_reward[catched_ball_mask] = 1.1 / (1.0 + tennisball_linear_vel_mag[catched_ball_mask])
            gravity_compensation_reward[catched_ball_mask] = 1.1 * gravity_compensation[catched_ball_mask]
            # termination_penalty[catched_ball_mask] *= 1.1
            alive_reward[catched_ball_mask] += 0.2
        
        # print("self.joint_acc_penalty", self.joint_acc_penalty.shape)
        # print("self.joint_jerk_penalty", self.joint_jerk_penalty.shape)
        rewards = (
            dist_reward_scale * dist_reward
            # + orient_reward_scale * orient_reward
            # + r_v_reward * relative_velocity_reward_scale
            + stability_reward * stability_reward_scale
            # + gravity_compensation_reward * gravity_compensation_reward_scale
            + dist_to_target_pos_reward * dist_to_target_pos_reward_scale
            + to_throwing_vel_reward * to_throwing_vel_reward_scale
            + to_throwing_pos_reward * to_throwing_pos_reward_scale
            # + task_reward * task_reward_scale
            - termination_penalty
            # - dof_at_limit_cost * dof_at_limit_cost_scale
            # - dof_vel_lim_penalty * dof_vel_lim_penalty_scale
            - action_change_penalty * action_rate_scale
        ...
        self.extras["log"] = {
            ...
            "r_v_reward": (r_v_reward * relative_velocity_reward_scale).mean(),
            "stability_reward": (stability_reward * stability_reward_scale).mean(),
            "gravity_compensation_reward": (gravity_compensation_reward * gravity_compensation_reward_scale).mean(),
            "dist_to_final_target_pos_reward": (dist_to_target_pos_reward * dist_to_target_pos_reward_scale).mean(),
            "to_throwing_vel_reward": (to_throwing_vel_reward * to_throwing_vel_reward_scale).mean(),
            "to_throwing_pos": (to_throwing_pos_reward * to_throwing_pos_reward_scale).mean(),
            "task_reward": (task_reward * task_reward_scale).mean(),
            ...
            "action_change_penalty": (-action_change_penalty * action_rate_scale).mean(),
            ...
            # "action_penalty": (action_penalty * 0.01).mean(),
    ...
    def random_throw(
        ...
        rand_x_final = - torch.rand(N, device=device) * x_range * 2.0
        ...
        z_final = final_state[:, 2] + rand_z_final
    ...
    def lowpass_filter(self, data, cutoff, fs):
        # Determine the device of the input data
        device = data.device

        # Perform FFT
        data_fft = torch.fft.fft(data, dim=0)
        freqs = torch.fft.fftfreq(data.size(0), d=1/fs).to(device)  # Ensure freqs is on the same device

        # Create a low-pass filter mask
        filter_mask = (torch.abs(freqs) <= cutoff).to(device)  # Ensure filter_mask is on the same device

        # Apply the filter mask
        data_fft_filtered = data_fft * filter_mask.unsqueeze(1)

        # Perform inverse FFT to get the filtered signal
        filtered_data = torch.fft.ifft(data_fft_filtered, dim=0).real
