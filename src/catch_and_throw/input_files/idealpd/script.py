...
class Lbr_iiwaSixStatesEnvCfg(DirectRLEnvCfg):
    ...
    joint_accel_scale = 1.0e-5
    joint_jerk_scale = 2.0e-7
    to_throwing_vel_reward_scale = 2.0
    ...
    action_noise_model: NoiseModelWithAdditiveBiasCfgActionv2 = NoiseModelWithAdditiveBiasCfgActionv2(
        noise_cfg=GaussianNoiseCfg(mean=0.0, std=0.003, operation="add"),
...
class Lbr_iiwaSixStatesEnv(DirectRLEnv):
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        conditions = [
            ...
            torch.any((self.actions - self.last_actions).abs() > 0.3, dim=-1),
            self.end_effector_pos[:, 2] < 0.30,
            torch.logical_and(self.rel_position_termination < 0, self.tennisball_pos[:, 0] < 1.0),
    ...
    def _get_observations(self) -> dict:
        ...
        condition_final_target = (self.task[:, -2] == 1.0) | (self.task[:, -3] == 1.0)
        condition_throwing = (self.task[:, -1] == 1.0)


        self.to_final_target = torch.where(
            condition_final_target.unsqueeze(-1),  # Shape: (num_envs, 1)
        ...
        self.to_throwing_vel = torch.where(
            condition_throwing.unsqueeze(-1),  # Shape: (num_envs, 1)
            (self.throwing_vel - self.tennisball_lin_vel),  # Shape: (num_envs, 1)
            torch.zeros((self.num_envs, 3), device=self.device)  # Shape: (num_envs, 1)
        )

        self.to_throwing_pos = torch.where(
            condition_throwing.unsqueeze(-1),  # Shape: (num_envs, 1)
            (self.throwing_pos - self.tennisball_pos),  # Shape: (num_envs, 1)
            torch.zeros((self.num_envs, 3), device=self.device)  # Shape: (num_envs, 1)
        )
    ...
    def _compute_rewards(
        ...
        if throwing_ball_mask.any():
            ...
            to_throwing_pos_reward[throwing_ball_mask] = 2.5 / (1.0 + 5.0 * torch.norm(self.throwing_pos[throwing_ball_mask] - self.tennisball_pos[throwing_ball_mask], p=2, dim=-1)**2)
    ...
    def random_throw_back(
        ...
        # vx = (final_state[:, 0]/2 - init_state[:, 0]) / t
        vx = torch.ones((N), device=self.device) * 1.0
        vy = (final_state[:, 1] - init_state[:, 1]) / t
        vz = torch.ones((N), device=self.device) * 0.5
