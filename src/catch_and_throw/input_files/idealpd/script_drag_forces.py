...
class EventCfg:
    ...
    end_effector_material = EventTerm(
        ...
        params={
            ...
            "restitution_range": (0.8, 0.95),          # End effector restitution (lower for grip)
            "num_buckets": 250,
        },
    )
    ...
    tennis_ball_material = EventTerm(
        ...
        params={
            ...
            "restitution_range": (0.8, 0.95),          # Tennis ball restitution (higher for bounce)
            "num_buckets": 250,
        },
    )
...
class Lbr_iiwaSixStatesEnv(DirectRLEnv):
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        ball_area = np.pi * self.cfg.tennisball_radius**2
    ...
    def _get_observations(self) -> dict:
        ...
        data_set = torch.cat(
            (   self._robot.data.applied_torque,
                ...
                self._robot.data.joint_vel + (torch.rand_like(self.joint_vel) * 2 - 1) * 0.01,  # 7
                # self.joint_vel,
                self.tennisball_pos + (torch.rand_like(self.tennisball_pos) * 2 - 1) * 0.005,  # 3
                self.tennisball_lin_vel + (torch.rand_like(self.tennisball_lin_vel) * 2 - 1) * 0.01,  # 3
                self.end_effector_pos + (torch.rand_like(self.end_effector_pos) * 2 - 1) * 0.005,  # 3
                self.end_effector_rot,  # 3
                self.end_effector_lin_vel + (torch.rand_like(self.end_effector_lin_vel) * 2 - 1) * 0.005,  # 4
                obs_task,  # 5
    ...
    def _compute_intermediate_values(self, env_ids: torch.Tensor | None = None):
        ...
        self.bouncing_velocity_threshold = 4.0 - 3.5 * curriculum_progress  # 5.0 -> 2.0
        ...
        high_bouncing = (distance < 0.12) & (self.rel_position_termination < (self.cfg.tennisball_radius * 1.5 + torch.rand_like(self.rel_position_termination) * 0.01)) & (relative_vel_magnitude >= self.bouncing_velocity_threshold)
    ...
    def _compute_rewards(
        ...
        r_v = torch.norm(end_effector_lin_vel - torch.exp(-1.6 * d).unsqueeze(-1) * tennisball_lin_vel_abs, p=2, dim=-1)
        rel_vel = end_effector_lin_vel - tennisball_lin_vel
        distance_weight = torch.exp(-0.8 * d)
        ...
        r_v_reward = 1.0 * (r_v_weight) * dist_reward
        ...
        rel_acc  = self.end_effector_lin_acc - torch.exp(-1.6 * d).unsqueeze(-1) * self.tennisball_lin_acc
        rel_acc_norm = torch.norm(rel_acc, p=2, dim=-1)
        r_a_weight = torch.exp(-0.4*rel_acc_norm)
        acc_pen  = 1.0 * r_a_weight * dist_reward
        ...
        if throwing_ball_mask.any():
            task_reward[stabilied_at_target_reward_mask] += 25.0
        ...
        if final_target_reached_mask.any():
            task_reward[final_target_reached_reward_mask] += 20.0
        ...
        if stabilized_ball_mask.any():
            task_reward[catched_ball_reward_mask] += 10.0
    ...
    def random_throw(
        ...
        y_final = torch.rand(N, device=device) * 0.5 - 0.6          # [-0.75, -0.1]
