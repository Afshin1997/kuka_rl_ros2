...
class Lbr_iiwaSixStatesEnvCfg(DirectRLEnvCfg):
    ...
    dist_to_target_pos_reward_scale = 1.0
    dof_at_limit_cost_scale = 0.2
    dof_vel_lim_penalty_scale = 0.2
    dof_acc_lim_penalty_scale = 0.2
    joint_accel_scale = 3.0e-4
    joint_jerk_scale = 2.0e-7
    joint_torque_scale = 1.2e-2
...
class Lbr_iiwaSixStatesEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: Lbr_iiwaSixStatesEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.effort_limit = 0.9 * torch.tensor([176, 176, 110, 110, 110, 40, 40], dtype=torch.float, device=self.device).repeat((self.num_envs, 1))
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        targets = self.robot_dof_pos + self.actions * 0.09
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        conditions = [
            ...
            # torch.any((self._robot.data.joint_acc).abs() > 13.0, dim=-1),
            # torch.any((self.joint_jerk).abs() > 600, dim=-1),
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        return self._compute_rewards(
            ...
            self.cfg.dof_acc_lim_penalty_scale,
    ...
    def _get_observations(self) -> dict:
        ...
        # print(EventCfg.actuator.stiffness)
               

        condition_final_target = (self.task[:, -3] == 1.0) | (self.task[:, -4] == 1.0)
        condition_throwing = (self.task[:, -2] == 1.0)
        ...
        # print(self.end_effector_pos.shape)
        # print(self.action_history_critic.flatten(start_dim=1)[0,:])
        # print(self._robot.data.joint_stiffness)
        obs_critic = torch.cat(
            (   self.pure_actions,
                # self._robot.data.joint_stiffness * 0.003,
                # self._robot.data.joint_damping * 0.04,
                ...
                self.joint_jerk * self.cfg.dof_jerk_scale,
                self.tennisball_pos * self.cfg.tennis_ball_pos_scale, # 3
                self.end_effector_pos, #3
    ...
    def _compute_intermediate_values(self, env_ids: torch.Tensor | None = None):
        ...
        #######################
        # self.joint_acc_obs = self._robot.data.joint_acc
        # self.joint_stiffness = self._robot.data.joint_stiffness
        #################
        ...
        self.moving_to_target_pos = (self.stabilized_ball_counter > 150) & distance_condition & rel_velocity_condition_to_taret
        ...
        self.throwing_ball = (self.stabilized_ball_counter_at_target > 100)
    ...
    def _compute_rewards(
        ...
        # self.joint_jerk_penalty = torch.sum(self.joint_jerk**2 + 2 * ((self.joint_jerk.abs() - 1000).clamp(min=0))**2, dim=-1)
        ...
        if stop_manipulator.any():
            dist_reward[stop_manipulator] = 2.1
            stability_reward[stop_manipulator] = 2.1
            dist_to_target_pos_reward[stop_manipulator] = 6.0
            to_throwing_pos_reward[stop_manipulator] = 15
            alive_reward[stop_manipulator] += 8.2
            joint_vel_penalty[stop_manipulator] =  5.0 / (1.0 + 2.5 * torch.norm(self.joint_vel[stop_manipulator], p=2, dim=-1)**2)
        ...
        if throwing_ball_mask.any():
            ...
            alive_reward[throwing_ball_mask] += 5.2
            ...
            to_throwing_pos_reward[throwing_ball_mask] = 2.5 / (1.0 + 1.0 * torch.norm(self.throwing_pos[throwing_ball_mask] - self.tennisball_pos[throwing_ball_mask], p=2, dim=-1)**2)
    ...
    def random_throw_back(
        ...
        vx = torch.ones((N), device=self.device) * 2.5
