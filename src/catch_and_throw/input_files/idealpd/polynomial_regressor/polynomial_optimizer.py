...
class PolynomialJointMapper:
    ...
    def load_data(self, folder_path):
        """Load all CSV files from the specified folder and create sequential data."""
        all_X = []
        all_y = []
        ...
        for file_path in csv_files:
            ...
            # X[i] = [joint_pos[i], set_target[i]]
            # y[i] = joint_pos[i+1]
            for i in range(len(df) - 1):
                # Concatenate joint positions and set targets for current row
                X_row = np.concatenate([joint_positions[i], set_targets[i]])
                # Next row's joint positions are the target
        ...
        if not all_X:
            raise ValueError(f"No valid sequences found in {folder_path}")
        
        # Convert lists to numpy arrays
        self.X = np.array(all_X)
        self.y = np.array(all_y)
        
        print(f"Loaded {len(self.X)} sequences from {len(list(Path(folder_path).glob('*.csv')))} files")
        print(f"X shape: {self.X.shape} (samples, features)")
        print(f"y shape: {self.y.shape} (samples, targets)")
...
def main():
    ...
    configs = [
        {'regularization': 'none', 'include_interaction': True, 'max_degree':3},
        {'regularization': 'ridge', 'include_interaction': True, 'max_degree': 3},
