...
class EventCfg:
    ...
    #         "static_friction_range": (0.6, 0.75),
    ...
    #         "static_friction_range": (0.59, 0.71),
    #         "dynamic_friction_range": (0.59, 0.61),
    #         "restitution_range": (0.95, 0.98),  # Changed to higher range for bouncing
    #         "num_buckets": 250,
    #     },
    # )
    ...
    end_effector_material = EventTerm(
        ...
        params={
            "asset_cfg": SceneEntityCfg("robot", body_names="iiwa_link_7.*"),
            "static_friction_range": (0.3, 0.8),      # End effector friction range
            "dynamic_friction_range": (0.2, 0.7),     # End effector friction range  
            "restitution_range": (0.5, 0.7),          # End effector restitution (lower for grip)
            "num_buckets": 250,
        },
    )
    ...
    tennis_ball_material = EventTerm(
        ...
        params={
            "asset_cfg": SceneEntityCfg("tennisball", body_names=".*"),
            "static_friction_range": (0.4, 0.9),      # Tennis ball friction range
            "dynamic_friction_range": (0.3, 0.8),     # Tennis ball friction range
            "restitution_range": (0.5, 0.7),          # Tennis ball restitution (higher for bounce)
            "num_buckets": 250,
        },
    )
    ...
    tennisball = RigidObjectCfg(
        ...
        spawn=sim_utils.SphereCfg(
            ...
            physics_material=RigidBodyMaterialCfg(
                ...
                restitution=0.75,  # Increased to near-perfect bounce
...
class Lbr_iiwaSixStatesEnv(DirectRLEnv):
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        # estimated_catching_pos = torch.tensor([0.2, -0.5, 0.9], dtype=torch.float, device=self.device).repeat(len(env_ids), 1)
        ...
        # self.tennisball_default_state[:, 0:3] += self.scene.env_origins[env_ids]
        # self.tennisball_default_state[:, 7:10] = self.tennisball_default_state[:, :3]
        # print("self.tennisball_default_state[:, 0:3]", self.tennisball_default_state[:, 0:3])
        # print("self.tennisball_default_state[:, 7:10]", self.tennisball_default_state[:, 0]+ self.scene.env_origins[env_ids, 0])
        ...
        # print("tennisball_pos",self.tennisball_pos[env_ids])
        # print("self.tennisball_lin_vel", self.tennisball_lin_vel[env_ids])
    ...
    def _get_observations(self) -> dict:
        ...
        obs_actor = torch.cat(
            (   self.action_history_actor.flatten(start_dim=1), #14
                ...
                (self.joint_vel + torch.randn_like(self.joint_vel) * 0.005) * self.cfg.dof_velocity_scale,
                # self.joint_vel * self.cfg.dof_velocity_scale, # 7
                (self.tennisball_pos + torch.randn_like(self.tennisball_lin_vel) * 0.002) * self.cfg.tennis_ball_pos_scale, # 3
                (self.tennisball_lin_vel + torch.randn_like(self.tennisball_lin_vel) * 0.01) * self.cfg.lin_vel_scale,
                # self.tennisball_lin_vel * self.cfg.lin_vel_scale, #3
                # self.end_effector_lin_vel * self.cfg.lin_vel_scale,
                (self.end_effector_lin_vel + torch.randn_like(self.end_effector_lin_vel) * 0.005) * self.cfg.lin_vel_scale,
                self.end_effector_rot,
            ),
            dim=-1,
        )
    ...
    def _compute_intermediate_values(self, env_ids: torch.Tensor | None = None):
        ...
        if can_revert_to_catching_ball.any():
            ...
            self.bouncing_happens[can_revert_to_catching_ball] = True
    ...
    def _compute_rewards(
        ...
        r_v = torch.norm(end_effector_lin_vel - tennisball_lin_vel, p=2, dim=-1)
        distance_weight = torch.exp(-2.0 * d**2)
        r_v_weight = torch.exp(-0.5 * r_v**2)
        r_v_reward = distance_weight * r_v_weight + 0.1 * r_v_weight 
        ...
        if stop_manipulator.any():
            dist_reward[stop_manipulator] = 1.6 * 4
        ...
        if throwing_ball_mask.any():
            dist_reward[throwing_ball_mask] = 1.6 * 4
            ...
            to_throwing_pos_reward[throwing_ball_mask] = 1.5 / (1.0 + 0.75 * torch.norm(self.throwing_pos[throwing_ball_mask] - self.tennisball_pos[throwing_ball_mask], p=2, dim=-1)**2)
    ...
    def random_throw_back(
        ...
        vx = torch.ones((N), device=self.device) * 4.0
    ...
    # def find_ball_intersection_at_plane_separate(
    #     self,
    #     init_pos,     # (N, 3) = [[x0, y0, z0], [x0, y0, z0], ...]
    #     init_vel,     # (N, 3) = [[vx0, vy0, vz0], [vx0, vy0, vz0], ...]
    #     plane_x=0.0,  # x-coordinate of the plane
    #     g=9.81,       # gravitational acceleration
    #     device=None
    # ):
    #     """
    #     Calculate the position and velocity of the ball when it intersects the plane x = plane_x.
    #     Returns separate tensors for position and velocity.
        
    #     Returns:
    #         intersection_pos: (N, 3) tensor containing [x, y, z] at intersection
    #         intersection_vel: (N, 3) tensor containing [vx, vy, vz] at intersection
    #         intersection_time: (N,) tensor containing time to intersection
    #         valid_mask: (N,) boolean tensor indicating valid intersections
    #     """
    ...
    #     vx0, vy0, vz0 = init_vel[:, 0], init_vel[:, 1], init_vel[:, 2]
        
    #     # Initialize output tensors
    #     intersection_pos = torch.zeros((N, 3), device=device)
    #     intersection_vel = torch.zeros((N, 3), device=device)        
    #     # t_intersection = torch.zeros(N, device=device)
    #     t_intersection = (plane_x - x0) / vx0
        
    #     # Position at intersection
    #     intersection_pos[:, 0] = plane_x  # x position (on the plane)
    #     intersection_pos[:, 1] = y0[:] + vy0[:] * t_intersection
    #     intersection_pos[:, 2] = (z0[:] + vz0[:] * t_intersection - 0.5 * g * t_intersection * t_intersection)
        
    #     # Velocity at intersection
    #     intersection_vel[:, 0] = vx0[:]
    #     intersection_vel[:, 1] = vy0[:]
    #     intersection_vel[:, 2] = vz0[:] - g * t_intersection

        
    #     return intersection_pos, intersection_vel