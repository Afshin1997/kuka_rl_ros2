...
class EKFWithKnownGravity:
    def __init__(self, dt, initial_state, process_noise_std, measurement_noise_std, gravity_camera_frame, device):
        """
        ...
        self.g = torch.tensor(gravity_camera_frame, dtype=torch.float32, device=device).unsqueeze(1)

        # State vector: [X, Y, Z, Vx, Vy, Vz]
        self.x = torch.tensor(initial_state, dtype=torch.float32, device=device).unsqueeze(1)
        ...
        self.B = torch.zeros((6, 3), dtype=torch.float32, device=device)
        self.B[3:, :] = dt * torch.eye(3, dtype=torch.float32, device=device)

        # Process noise covariance (Q)
        pos_noise_std, vel_noise_std = process_noise_std
        q_pos = pos_noise_std ** 2
        q_vel = vel_noise_std ** 2

        self.Q = torch.zeros((6, 6), dtype=torch.float32, device=device)
        self.Q[0:3, 0:3] = torch.eye(3, device=device) * q_pos
        self.Q[3:6, 3:6] = torch.eye(3, device=device) * q_vel

        # Measurement matrix (H)
        self.H = torch.zeros((3, 6), dtype=torch.float32, device=device)
        self.H[0:3, 0:3] = torch.eye(3, device=device)

        # Measurement noise covariance (R)
        r = measurement_noise_std ** 2
        self.R = r * torch.eye(3, device=device)

        # State covariance matrix (P)
        self.P = torch.eye(6, dtype=torch.float32, device=device) * 1.0  # Initial uncertainty
...
class DepthColorCamera:
    def __init__(self):
        ...
        # Retrieve extrinsic calibration between IMU and depth camera
        profile = self.pipeline.get_active_profile()
        accel_stream = profile.get_stream(rs.stream.accel).as_motion_stream_profile()
        depth_stream = profile.get_stream(rs.stream.depth).as_video_stream_profile()
        self.extrinsics_imu_to_depth = accel_stream.get_extrinsics_to(depth_stream)
    ...
    def get_frames(self):
        ...
        accel_data = np.zeros(3, dtype=np.float32)
        gyro_data = np.zeros(3, dtype=np.float32)
        if accel_frame:
        ...
        return depth_image, color_image, accel_data, gyro_data
...
# ---------------------------
def ekf_prediction_loop(ekf, stop_event, prediction_writer, prediction_interval=1.0 / 240.0):
...
def process_image_for_ball_tracking(color_image, depth_image, camera_matrix, dist_coeffs):
    ...
    mask_smooth = cv2.GaussianBlur(mask_clean, (7, 7), 0)
...
# ---------------------------
def convert_to_world_coordinates(x_camera, y_camera, z_camera, R_camera_to_world):
    camera_coords = np.array([x_camera, y_camera, z_camera])
    world_coords = R_camera_to_world @ camera_coords
    return world_coords
...
# EKF Initialization
# ---------------------------
def ekf_initialization(gyro_data, accel_data, gyro_data_buffer, acc_data_buffer, num_samples, sampling_rate, rotation_depth_to_imu):
    # Append gyro and accel data to buffers
    gyro_data_buffer.append(gyro_data)
    acc_data_buffer.append(accel_data)

    if len(gyro_data_buffer) > num_samples:
        gyro_data_buffer.pop(0)
    if len(acc_data_buffer) > num_samples:
        acc_data_buffer.pop(0)

    ekf = None
    ekf_initialized = False
    R_camera_to_world = None
    rotation_camera_to_world = None

    if len(gyro_data_buffer) == num_samples and len(acc_data_buffer) == num_samples:
        gyro_array = np.array(gyro_data_buffer)
        acc_array = np.array(acc_data_buffer)

        madgwick = Madgwick(gyr=gyro_array, acc=acc_array, frequency=sampling_rate)
        last_quat = madgwick.Q[-1]  # (w, x, y, z)

        # Convert quaternion to rotation matrix (IMU to World)
        rotation_imu_to_world = R.from_quat([last_quat[1], last_quat[2], last_quat[3], last_quat[0]]).as_matrix()

        # Calculate the camera to world rotation matrix
        rotation_camera_to_world = rotation_imu_to_world @ rotation_depth_to_imu

        # Gravity vector in the world frame (standard is along negative z-axis)
        g_world = np.array([0, 0, -9800])
        g_camera = rotation_camera_to_world.T @ g_world

        # Set up initial state and EKF
        initial_position = [800, 4500, -1000]
        initial_velocity = [-0, -3500, 0]
        initial_state = np.concatenate((initial_position, initial_velocity))

        process_noise_std = (100.0, 10.0)
        measurement_noise_std = 30.0

        ekf = EKFWithKnownGravity(
            dt=1.0 / 240.0,
            initial_state=initial_state,
            process_noise_std=process_noise_std,
            measurement_noise_std=measurement_noise_std,
            gravity_camera_frame=g_camera,
            device=torch.device('cpu')
        )
        ekf_initialized = True

    return ekf, ekf_initialized, rotation_camera_to_world, gyro_data_buffer, acc_data_buffer
...
def main():
    ...
    # Calculate rotation from depth to IMU frame using extrinsic calibration
    extrinsics_imu_to_depth = dc.extrinsics_imu_to_depth
    rotation_imu_to_depth = np.array(extrinsics_imu_to_depth.rotation).reshape(3, 3)
    translation_imu_to_depth = np.array(extrinsics_imu_to_depth.translation)

    # Invert to get rotation from depth to IMU
    rotation_depth_to_imu = rotation_imu_to_depth.T
    translation_depth_to_imu = -rotation_depth_to_imu @ translation_imu_to_depth
    ...
    max_frames_without_ball = 3  # Stop recording immediately when the ball is not detected

    try:
        while True:
            depth_image, color_image, accel_data, gyro_data = dc.get_frames()

            if not ekf_initialized:
                ekf, ekf_initialized, R_camera_to_world, gyro_data_buffer, acc_data_buffer = ekf_initialization(
                    gyro_data, accel_data, gyro_data_buffer, acc_data_buffer, num_samples, sampling_rate, rotation_depth_to_imu
                )

            if ekf_initialized and color_image is not None and depth_image is not None:
                ...
                if ball_detected:
                    ...
                    x_world, y_world, z_world = convert_to_world_coordinates(
                        x_3d, y_3d, z_3d, R_camera_to_world
                    )
                    ...
                    if y_world < 4500:
                        ...
                        if prev_position is not None and prev_time is not None:
                            ...
                            vx = (x_3d - prev_position[0]) / dt
                            vy = (y_3d - prev_position[1]) / dt
                            vz = (z_3d - prev_position[2]) / dt
                            ...
                            if ekf_thread is None:
                                ...
                                ekf_thread = threading.Thread(target=ekf_prediction_loop,
                                                              args=(ekf, stop_event, prediction_writer, 1.0 / 240.0))
                                ekf_thread.start()

                            z_state = torch.tensor([x_world, y_world, z_world], dtype=torch.float32)
                            print("z_state", x_world, y_world, z_world)
                            ekf.update(z_state)
