...
class EKFWithKnownGravity:
    def __init__(self, dt):
        ...
        q_a = 1e-3  # Adjust based on expected acceleration noise
        dt = self.dt
        Q_block = torch.tensor([
            [dt**4 / 4 * q_a, dt**3 / 2 * q_a],
            [dt**3 / 2 * q_a, dt**2 * q_a]
        ], dtype=torch.float32)

        self.Q = torch.zeros((6, 6), dtype=torch.float32)
        self.Q[0, 0] = Q_block[0, 0]  # X position variance
        self.Q[0, 3] = Q_block[0, 1]  # Covariance between X position and X velocity
        self.Q[3, 0] = Q_block[1, 0]
        self.Q[3, 3] = Q_block[1, 1]  # X velocity variance

        self.Q = torch.zeros((6, 6), dtype=torch.float32)
        self.Q[1, 1] = Q_block[0, 0]  # X position variance
        self.Q[1, 4] = Q_block[0, 1]  # Covariance between X position and X velocity
        self.Q[4, 1] = Q_block[1, 0]
        self.Q[4, 4] = Q_block[1, 1]  # X velocity variance

        self.Q = torch.zeros((6, 6), dtype=torch.float32)
        self.Q[2, 2] = Q_block[0, 0]  # X position variance
        self.Q[2, 5] = Q_block[0, 1]  # Covariance between X position and X velocity
        self.Q[5, 2] = Q_block[1, 0]
        self.Q[5, 5] = Q_block[1, 1]  # X velocity variance

        # Measurement matrix (H)
        self.H = torch.zeros((3, 6), dtype=torch.float32)
        self.H[0, 0] = 1  # Measure X position
        self.H[1, 1] = 1  # Measure Y position
        self.H[2, 2] = 1  # Measure Z position
        ...
        self.P = torch.zeros((6, 6), dtype=torch.float32)
        self.P[0:3, 0:3] = torch.eye(3) * 1e-2  # Low uncertainty in positions
        self.P[3:6, 3:6] = torch.eye(3) * 1e2   # Higher uncertainty in velocities
    ...
    def update(self, z):
        ...
        y = z - self.H.mm(self.x)  # Measurement residual (3x1)
        S = self.H.mm(self.P).mm(self.H.t()) + self.R  # Innovation covariance (3x3)
        K = self.P.mm(self.H.t()).mm(torch.inverse(S))  # Kalman gain (6x3)
        self.x = self.x + K.mm(y)  # Updated state estimate (6x1)
        I = torch.eye(6, dtype=torch.float32, device=self.device)
        self.P = (I - K.mm(self.H)).mm(self.P)  # Updated state covariance (6x6)
        return self.x
...
def main():
    ...
    R_camera_to_world = np.load('required_files/camera_pose_inv.npz')['R_inv']
    T_camera_to_world = np.load('required_files/camera_pose_inv.npz')['tvec_inv']
