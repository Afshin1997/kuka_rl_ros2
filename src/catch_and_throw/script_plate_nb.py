...
class EventCfg:
    ...
    reset_gravity = EventTerm(
        ...
        mode="reset",
        params={
            "gravity_distribution_params": ([-0.1, -0.1, -0.3], [0.1, 0.1, 0.3]),
            "operation": "add",
            "distribution": "uniform",
        },
    )
    ...
    end_effector_material = EventTerm(
        ...
        params={
            ...
            "static_friction_range": (0.2, 0.4),      # End effector friction range
            "dynamic_friction_range": (0.15, 0.35),     # End effector friction range  
            "restitution_range": (0.7, 0.9),          # End effector restitution (lower for grip)
            "num_buckets": 250,
        },
    )
    ...
    tennis_ball_material = EventTerm(
        ...
        params={
            ...
            "static_friction_range": (0.2, 0.4),      # Tennis ball friction range
            "dynamic_friction_range": (0.15, 0.35),     # Tennis ball friction range
            "restitution_range": (0.7, 0.9),          # Tennis ball restitution (higher for bounce)
            "num_buckets": 250,
        },
    )
...
class Lbr_iiwaSixStatesEnvCfg(DirectRLEnvCfg):
    ...
    robot = ArticulationCfg(
        ...
        spawn=sim_utils.UsdFileCfg(
            ...
            articulation_props=sim_utils.ArticulationRootPropertiesCfg(
                enabled_self_collisions=True, solver_position_iteration_count=8, solver_velocity_iteration_count=1, sleep_threshold=0.005, stabilization_threshold=0.001,
            ),
    ...
    tennisball = RigidObjectCfg(
        ...
        spawn=sim_utils.SphereCfg(
            ...
            rigid_props=sim_utils.RigidBodyPropertiesCfg(
                ...
                disable_gravity=False,
                enable_gyroscopic_forces=True,
                solver_position_iteration_count=8,  # Increased for better collision accuracy
                solver_velocity_iteration_count=1,  # Increased for better collision accuracy
            ...
            physics_material=RigidBodyMaterialCfg(
                ...
                compliant_contact_stiffness=0.0,  # Keep at 0 for rigid contacts
                compliant_contact_damping=0.0,  # Keep at 0 to avoid energy dissipation
            ),
        ),
    ...
    observation_noise_model: NoiseModelWithAdditiveBiasCfgObs = NoiseModelWithAdditiveBiasCfgObs(
        ...
        bias_noise_cfg=GaussianNoiseCfg(mean=0.0, std=0.003, operation="abs"),
    )
...
class Lbr_iiwaSixStatesEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: Lbr_iiwaSixStatesEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.init_dof_pos_scaled = (
            2.0
            * (self.robot_dof_targets - self.robot_dof_lower_limits)
        ...
        self.given_task_reward = {
            ...
            'catching_ball': torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)
        }
        ...
        # self.joint_pos_history = self.init_dof_pos_scaled.unsqueeze(1).repeat((1, 4, 1))
        # self.ball_pos_history = torch.tensor([3.0, -0.45, 0.25], dtype=torch.float, device=self.device).unsqueeze(1).repeat((self.num_envs, 3, 1))
        # self.ball_pos_history = torch.tensor([3.0, -0.45, 0.25], dtype=torch.float, device=self.device).unsqueeze(0).unsqueeze(0).repeat(self.num_envs, 4, 1)
        ...
        self.max_curriculum_level = 1000  # After this, threshold is fixed
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        ball_area = torch.pi * self.cfg.tennisball_radius**2
        
        # Drag force: F_d = 0.5 * ρ * v² * C_d * A
        vel_magnitude = torch.norm(self.tennisball_lin_vel, p=2, dim=-1, keepdim=True)
        drag_coefficient = 0.45  # sphere drag coefficient
        drag_force = -0.5 * air_density * vel_magnitude * self.tennisball_lin_vel * drag_coefficient * ball_area
        
        # Magnus force (spin effect): F_m = 0.5 * ρ * v * C_l * A * (ω × v)
        magnus_coefficient = 0.25  # typical for tennis ball
        # print(self.tennisball_ang_vel.shape, self.tennisball_lin_vel.shape)
        magnus_force = 0.5 * air_density * vel_magnitude * magnus_coefficient * ball_area * \
                    torch.cross(self.tennisball_ang_vel, self.tennisball_lin_vel, dim=-1)
        
        total_forces = drag_force + magnus_force

        self.aerodynamic_forces = total_forces.unsqueeze(1)  # Shape: [num_envs, 1, 3]
    
        # Optional: No external torques for now, but you could add them
        self.aerodynamic_torques = torch.zeros_like(self.aerodynamic_forces)

    def _apply_action(self):
        self._robot.set_joint_effort_target(self.torque_target)
        self._tennisball.set_external_force_and_torque(forces=self.aerodynamic_forces, torques=self.aerodynamic_torques)
        self._robot.write_data_to_sim()
        self._tennisball.write_data_to_sim()
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        conditions = [
            ...
            torch.any((self.pure_actions.abs() > 0.96), dim=-1),
            self.end_effector_pos[:, 2] < 0.25,
            # self.tennisball_pos[:, 2] < 0.2,
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        self.tennisball_default_state[:, 10:] = (torch.rand_like(self.init_ball_state) * 2 - 1) * 1.0
        ...
        # self.revert_penalty[env_ids] = torch.zeros_like(env_ids, dtype=torch.float, device=self.device)
        ...
        # self.joint_pos_history[env_ids] = self.init_dof_pos_scaled[env_ids].unsqueeze(1).repeat(1, 4, 1)
        # self.ball_pos_history[env_ids] = self.init_ball_state.unsqueeze(1).repeat(1, 4, 1)
        # print(self.ball_pos_history.shape)
        # print("here")
    ...
    def _get_observations(self) -> dict:
        ...
        #         self._robot.data.joint_vel + (torch.rand_like(self.joint_vel) * 2 - 1) * 0.03,  # 7
        #         # self.joint_vel,
        #         self.tennisball_pos + (torch.rand_like(self.tennisball_pos) * 2 - 1) * 0.01,  # 3
        #         self.tennisball_lin_vel + (torch.rand_like(self.tennisball_lin_vel) * 2 - 1) * 0.1,  # 3
        #         self.end_effector_pos + (torch.rand_like(self.end_effector_pos) * 2 - 1) * 0.01,  # 3
        #         self.end_effector_rot,  # 3
        #         self.end_effector_lin_vel + (torch.rand_like(self.end_effector_lin_vel) * 2 - 1) * 0.03,  # 4
        #         obs_task,  # 5
        ...
        # self.joint_pos_history[:, -1] = self.dof_pos_scaled.clone()
        # self.ball_pos_history = torch.roll(self.ball_pos_history, shifts=-1, dims=1)
        # self.ball_pos_history[:, -1] = self.tennisball_pos.clone()

        # print("self.joint_pos_history:", self.joint_pos_history[0])
        # print("self.ball_pos_history:", self.ball_pos_history[0])
        # print("self.action_history_actor", self.action_history_actor[0])

        # print("###################")
        
        # obs_actor = torch.cat(
        #     (   self.action_history_actor.flatten(start_dim=1), #28
        #         # self.dof_pos_scaled, # 7
        #         (self.joint_pos_history + (torch.rand_like(self.joint_pos_history) * 2 - 1) * 0.003).flatten(start_dim=1), # 28
        ...
        #         # (self.tennisball_pos + (torch.rand_like(self.tenn # row_counter = {env: 0 for env in range(num_record_envs)}
        # max_rows_to_save = 2000  # Set the limit to 50 rows
        ...
        #         writer = csv.writer(csvfile)
        #         writer.writerow(headers)
        #         writer.writerows(data_array)isball_pos) * 2 - 1) * 0.005) * self.cfg.tennis_ball_pos_scale, # 3
        #         (self.ball_pos_history + (torch.rand_like(self.ball_pos_history) * 2 - 1) * 0.005).flatten(start_dim=1) * self.cfg.tennis_ball_pos_scale, # 12
        ...
        #         (self.end_effector_pos + (torch.rand_like(self.end_effector_pos) * 2 - 1) * 0.005), # 3
        #         # self.end_effector_lin_vel * self.cfg.lin_vel_scale,
        #         (self.end_effector_lin_vel + (torch.rand_like(self.end_effector_lin_vel) * 2 - 1) * 0.005) * self.cfg.lin_vel_scale, #3
        ...
        obs_actor = torch.cat(
            (   self.action_history_actor.flatten(start_dim=1), #14
                self.dof_pos_scaled, # 7
                (self.joint_vel + (torch.rand_like(self.joint_vel) * 2 - 1) * 0.03) * self.cfg.dof_velocity_scale,
                (self.tennisball_pos + (torch.rand_like(self.tennisball_pos) * 2 - 1) * 0.01) * self.cfg.tennis_ball_pos_scale, # 3
                (self.tennisball_lin_vel + (torch.rand_like(self.tennisball_lin_vel) * 2 - 1) * 0.1) * self.cfg.lin_vel_scale,
                (self.end_effector_pos + (torch.rand_like(self.end_effector_pos) * 2 - 1) * 0.005),
                (self.end_effector_lin_vel + (torch.rand_like(self.end_effector_lin_vel) * 2 - 1) * 0.03) * self.cfg.lin_vel_scale,
                self.end_effector_rot,
            ),
            dim=-1,
        )
    ...
    def _compute_intermediate_values(self, env_ids: torch.Tensor | None = None):
        ...
        self.tennisball_ang_vel = self._tennisball.data.root_ang_vel_w
        ...
        self.bouncing_velocity_threshold = 4.0 - 2.75 * curriculum_progress  # 5.0 -> 2.0
        # self.bouncing_velocity_threshold = 1.1 - 0.1 * curriculum_progress

        relative_vel = self.end_effector_lin_vel - self.tennisball_lin_vel
        relative_vel_magnitude = torch.norm(relative_vel, p=2, dim=-1)
        velocity_factor = 1.0 - (1.0 / (1.0 + 0.2 * relative_vel_magnitude**2))
        ...
        cosine_angle_vel = F.cosine_similarity(self.end_effector_lin_vel, self.tennisball_lin_vel, dim=-1)
        ...
        if can_transition_to_stabilizing_ball.any():
            ...
            self.revert_penalty[can_transition_to_stabilizing_ball] = -(4.0 * velocity_factor[can_transition_to_stabilizing_ball])
           
        
        if can_revert_to_catching_ball.any():
            self.task[can_revert_to_catching_ball] = self.one_hot_encoding[catching_ball_index]
            self.revert_penalty[can_revert_to_catching_ball] = -(6.0 * velocity_factor[can_revert_to_catching_ball])
            if high_bouncing.any():
                # print(self.bouncing_happens)
                # self.revert_penalty[can_revert_to_catching_ball] = -2.0
                # penalty_scale = 1.0 + curriculum_progress[high_bouncing] * 2.0  # Penalty increases with curriculum
                # self.revert_penalty[high_bouncing] = -(6.0 * velocity_factor[high_bouncing])
        ...
        velocity_condition = torch.norm(self.tennisball_lin_vel, p=2, dim=-1) < 0.3
        rel_velocity_condition = torch.norm(self.tennisball_lin_vel - self.end_effector_lin_vel, p=2, dim=-1) < 0.3
        ...
        rel_velocity_condition_to_taret = torch.norm(self.tennisball_lin_vel - self.end_effector_lin_vel, p=2, dim=-1) < 0.3
        ...
        final_vel_cond = torch.norm(self.tennisball_lin_vel, p=2, dim=-1) < 0.3
        final_rel_velocity_condition = torch.norm(self.tennisball_lin_vel - self.end_effector_lin_vel, p=2, dim=-1) < 0.3
    ...
    def _compute_rewards(
        ...
        tennisball_lin_vel_abs = tennisball_lin_vel.clone()
        tennisball_lin_vel_abs[:, 2] = -torch.abs(tennisball_lin_vel[:, 2])
        # dist_reward = torch.zeros((self.num_envs), device=self.device)
        dist_reward = 1.0 * torch.exp(-1.6 * d)
        ...
        orientation_pure = F.cosine_similarity(-normalized_end_effector_z, tennisball_lin_vel_normalized, dim=-1)
        orient_reward = 0.4 * orientation_pure

        #Relative speed between end effecor and tennisball
        r_v = torch.norm(end_effector_lin_vel - torch.exp(-1.5 * d).unsqueeze(-1) * tennisball_lin_vel_abs, p=2, dim=-1)
        rel_vel = end_effector_lin_vel - tennisball_lin_vel
        distance_weight = torch.exp(-1.6 * d)
        r_v_weight = torch.exp(-0.8 * r_v)
        # alignment = torch.nn.functional.cosine_similarity(-rel_vel, tennisball_lin_vel, dim=-1)
        ee_x_v   = end_effector_lin_vel[:, 0]          # (N,)
        ball_x_v  = tennisball_lin_vel[:, 0]  # (N,)

        alignment_v = F.cosine_similarity(
            ee_x_v.unsqueeze(-1),           # (N,1)
            ball_x_v.unsqueeze(-1),
            dim=-1
        )
        alignment_weight_v = torch.exp(0.2 * alignment_v)
        r_v_reward = 0.5 * (r_v_weight) * distance_weight


        ee_z_acc   = self.end_effector_lin_acc          # (N,)
        ball_z_acc  = self.tennisball_lin_acc  # (N,)
        alignment_acc = F.cosine_similarity(
            ee_z_acc,           # (N,1)
            ball_z_acc,
            dim=-1
        )

        alignment_weight_acc = torch.exp(0.2 * alignment_acc)
        rel_acc  = self.end_effector_lin_acc - torch.exp(-1.5 * d).unsqueeze(-1) * self.tennisball_lin_acc
        rel_acc_norm = torch.norm(rel_acc, p=2, dim=-1)
        r_a_weight = torch.exp(-0.8*rel_acc_norm)
        acc_pen  = 1.0 * r_a_weight     
        ...
        catching_ball_reward_mask = catched_ball_mask & ~self.given_task_reward['catching_ball']
        ...
        gravity_compensation = F.cosine_similarity(normalized_end_effector_z, self.z_unit_tensor, dim=-1)


        if stop_manipulator.any():
            dist_reward[stop_manipulator] = 1.6
            ...
            alive_reward[stop_manipulator] += 1.2
            r_v_reward[stop_manipulator] = 1.0
            joint_vel_penalty[stop_manipulator] =  1.0 / (1.0 + 0.05 * torch.norm(self.joint_vel[stop_manipulator], p=2, dim=-1)**2)
            orient_reward[stop_manipulator] = 0.4
            acc_pen[stop_manipulator] = 1.0
            gravity_compensation_reward[stop_manipulator] = 2.0

        if throwing_ball_mask.any():
            task_reward[stabilied_at_target_reward_mask] += 5.0
            ...
            alive_reward[throwing_ball_mask] += 1.0
            r_v_reward[throwing_ball_mask] = 1.6
            to_throwing_vel_reward[throwing_ball_mask] = (1.5 / (1.0 + 2.0 * torch.norm(self.throwing_vel[throwing_ball_mask] - self.tennisball_lin_vel[throwing_ball_mask], p=2, dim=-1)**2))
            ...
            orient_reward[throwing_ball_mask] = 0.4
            # acc_pen[throwing_ball_mask] = 1.0
            # print("hhhhhhhhhhhhere")
            # print(orient_reward[throwing_ball_mask])
            gravity_compensation_reward[throwing_ball_mask] = 2.0


        if final_target_reached_mask.any():
            task_reward[final_target_reached_reward_mask] += 5.0
            self.given_task_reward['reached_final_target'][final_target_reached_reward_mask] = True
            dist_reward[final_target_reached_mask] = 1.6 * torch.exp(-1.6 * d[final_target_reached_mask])
            ...
            alive_reward[final_target_reached_mask] += 0.8
            # r_v_reward[final_target_reached_mask] = 1.6 * torch.exp(-0.8 * r_v[final_target_reached_mask])
            orient_reward[final_target_reached_mask] = 0.4
            # acc_pen[final_target_reached_mask] = 1.0
            gravity_compensation_reward[final_target_reached_mask] = 2 * gravity_compensation[final_target_reached_mask]
        ...
        if moving_to_target_mask.any():
            task_reward[stabilized_ball_reward_mask] += 5.0
            self.given_task_reward['stabilized_ball'][stabilized_ball_reward_mask] = True
            dist_reward[moving_to_target_mask] = 1.5 * torch.exp(-1.6 * d[moving_to_target_mask])
            ...
            alive_reward[moving_to_target_mask] += 0.6
            # r_v_reward[moving_to_target_mask] = 1.5 * torch.exp(-0.8 * r_v[moving_to_target_mask])
            orient_reward[moving_to_target_mask] = 0.4
            # acc_pen[moving_to_target_mask] = 1.0
            gravity_compensation_reward[moving_to_target_mask] = 2 * gravity_compensation[moving_to_target_mask]
        ...
        if stabilized_ball_mask.any():
            ...
            dist_reward[stabilized_ball_mask] = 1.3 * torch.exp(-1.6 * d[stabilized_ball_mask])
            stability_reward[stabilized_ball_mask] = 1.3 / (1.0 + tennisball_linear_vel_mag[stabilized_ball_mask])
            alive_reward[stabilized_ball_mask] += 0.4
            # r_v_reward[stabilized_ball_mask] = 1.3 * torch.exp(-0.8 * r_v[stabilized_ball_mask])
            orient_reward[stabilized_ball_mask] = 0.4
            # acc_pen[stabilized_ball_mask] = 1.0
            gravity_compensation_reward[stabilized_ball_mask] = 2 * gravity_compensation[stabilized_ball_mask]

        if catched_ball_mask.any():
            task_reward[catching_ball_reward_mask] += 5.0
            self.given_task_reward['catching_ball'][catching_ball_reward_mask] = True
            dist_reward[catched_ball_mask] = 1.1 * torch.exp(-1.6 * d[catched_ball_mask])
            stability_reward[catched_ball_mask] = 1.1 / (1.0 + tennisball_linear_vel_mag[catched_ball_mask])
            alive_reward[catched_ball_mask] += 0.2
            # r_v_reward[catched_ball_mask] = 1.1 * torch.exp(-0.8 * r_v[catched_ball_mask])
            orient_reward[catched_ball_mask] = 0.4
            # acc_pen[catched_ball_mask] = 1.0
            gravity_compensation_reward[catched_ball_mask] = 2 * gravity_compensation[catched_ball_mask]
        ...
        self.extras["log"] = {
            ...
            "gravity_compensation_reward": (gravity_compensation_reward).mean(),
    ...
    def random_throw(
        ...
        # ---------- target position ----------
        x_final = torch.rand(N, device=device) * 0.6 - 0.6          # [-0.75, 0.0]
        y_final = torch.rand(N, device=device) * 0.5 - 0.7          # [-0.75, -0.1]
        z_final = torch.rand(N, device=device) * 0.55 + 0.20          # [0.40, 1.05]
