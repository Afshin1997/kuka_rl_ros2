...
class BallTrajectoryPlotter:
    ...
    def plot_positions_and_velocities(self):
        ...
        # # Create additional figure for speed and acceleration
        # fig2, axes2 = plt.subplots(2, 1, figsize=(10, 8), sharex=True)
        # fig2.suptitle(f'Speed and Acceleration\n{os.path.basename(self.csv_file)}', 
        #               fontsize=14, fontweight='bold')
        
        # # Total speed
        # axes2[0].plot(time, self.df['speed'], 'm-', linewidth=2)
        # axes2[0].set_ylabel('Speed [m/s]')
        # axes2[0].set_title('Total Speed vs Time')
        # axes2[0].grid(True, alpha=0.3)
        
        # # Calculate and plot acceleration magnitude
        # if len(self.df) > 1:
        #     # Calculate acceleration using finite differences
        #     dt = np.diff(time)
        #     dvx = np.diff(self.df['vx_world'])
        #     dvy = np.diff(self.df['vy_world'])
        #     dvz = np.diff(self.df['vz_world'])
        ...
        #     # Plot acceleration (use time[1:] since diff reduces array size by 1)
        #     axes2[1].plot(time[1:], accel_mag, 'c-', linewidth=2)
        #     axes2[1].set_ylabel('Acceleration [m/s²]')
        #     axes2[1].set_xlabel('Time [s]')
        #     axes2[1].set_title('Acceleration Magnitude vs Time')
    ...
    def plot_separate_figures(self):
        ...
        # # Figure 2: Velocities only
        # fig2, axes2 = plt.subplots(1, 3, figsize=(15, 5))
        # fig2.suptitle(f'Ball Velocities\n{os.path.basename(self.csv_file)}', 
        #               fontsize=16, fontweight='bold')
        
        # # X velocity
        # axes2[0].plot(time, self.df['vx_world'], 'r--', linewidth=2.5)
        # axes2[0].set_xlabel('Time [s]')
        # axes2[0].set_ylabel('Vx [m/s]')
        # axes2[0].set_title('X Velocity vs Time')
        # axes2[0].grid(True, alpha=0.3)
        # axes2[0].axhline(y=0, color='k', linestyle=':', alpha=0.5)
        
        # # Y velocity
        # axes2[1].plot(time, self.df['vy_world'], 'g--', linewidth=2.5)
        # axes2[1].set_xlabel('Time [s]')
        # axes2[1].set_ylabel('Vy [m/s]')
        # axes2[1].set_title('Y Velocity vs Time')
        # axes2[1].grid(True, alpha=0.3)
        # axes2[1].axhline(y=0, color='k', linestyle=':', alpha=0.5)
        
        # # Z velocity
        # axes2[2].plot(time, self.df['vz_world'], 'b--', linewidth=2.5)
        # axes2[2].set_xlabel('Time [s]')
        # axes2[2].set_ylabel('Vz [m/s]')
        # axes2[2].set_title('Z Velocity vs Time')
